...
LDFLAGS = -L$(FFTWDIR)/lib -lfftw3 -lfftw3f
...
## need to be set.
#MPI = mpi2       # Use F77 header, more portable   ("include mpif.h")
MPI = usempi2    # Use Fortran 90 interface, safer ("use mpi")
#MPI = nompi
...
## which can be set via XCC, and corresponding flags via XCFLAGS; see the appropriate
...
## The libraries must be in the search path to compile with HDF5
## functionality. This can be achieved by either
##   a) compiling with the h5fc wrapper provided by the HDF5 library
## or
...
REQUIREDMODULES = gcc/4.4.7 intel/12.0.2 hdf5/1.8.13 mpich2/3.1.3-intel fftw/3.3.4
...
## compiling the Fortran source code. [If used, the internally built umfpack
## library will require a C compiler, but that compiler should be able to
## generate code for the target machine; see XCC below]. You may wish set
## something here if after building the code your executable name does not
## appear to contain any version information, which may be of some use.
#CC = gcc

## Set the C compiler that can generate binaries for the target machine.
## This is only used to build provided libraries when required
## (UMFPACK and AMD when implicit and nonspectral runs are desired).
## If CC is set above, XCC will take the same value. However, sometimes a
## different compiler is required.
#XCC = gcc

## Set the flags to be used with CC or XCC. If CFLAGS are set, XCFLAGS will
## use CFLAGS unless otherwise specified. These are just used of optimization
## or debugging of the included external libraries (AMD and UMFPACK).
#CFLAGS =
#XCFLAGS = -Ofast
