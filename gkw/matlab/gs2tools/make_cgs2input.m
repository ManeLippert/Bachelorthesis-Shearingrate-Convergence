...
% 
% The modification method is not very robust and relies on a fixed 
% format of the gs2 input file (e.g. the templates provided).
...
comsp{8}='  ! frame rotation u_parallel';
comsp{9}='  ! collisionality for this species, see below';
...
if (isfield(xpareq,'rhoc')) %% eik equilibrium input structure
    ...
    %fprintf(fptr,'%s\n', sss(is-2: end));
    fprintf(fptr,'%s',' alpha_input = ');
    fprintf(fptr,' %f', xpareq.alpha_input);
    fprintf(fptr,'%s\n', sss(is-2: end));
end
