...
% spectrum, useful for nonlinear runs.  This script assumes the naming
% conventions of the script gkwnlin, requires gkw_path.m to be setup.
% 
% fluxes_spec(proj,runname,flux,species,n_av,normalise)
%
% WHERE:
%
...
%
% EXAMPLE: fluxes_spec('proj','runname',2,2,300,1)
%
% The spectrum in file is plotted against  runname.krho values in /spectrum/kyspec


function []=fluxes_spec(proj,filename,flux,species,n_av,normalize)
...
av=av*nmod/scale(end,1);
error=error*nmod/scale(end,1);
...
errorbar(scale(1:nmod,1),av(start:finish),error(start:finish),'+-','DisplayName',label)
% Create xlabel
xlabel('ky.rho');
legend('hide')
legend('show')
